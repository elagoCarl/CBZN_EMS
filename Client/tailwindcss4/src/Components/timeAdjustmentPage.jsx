  ...
  const renderDetails = req => (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 text-sm sm:text-base">
      <div className="space-y-4">
        <h3 className="text-green-500 font-semibold text-sm sm:text-base mb-2">Time Adjustment Details</h3>
        <div><p className="text-xs sm:text-sm font-medium text-gray-400">Date</p><p className="text-white">{formatDate(req.date)}</p></div>
        <div><p className="text-xs sm:text-sm font-medium text-gray-400">Time Change</p><p className="text-white">From {formatTime(req.time_in)} to {formatTime(req.time_out)}</p></div>
        ...
        <div className="bg-[#363636] rounded-lg overflow-hidden flex flex-col">
          <div className="overflow-x-auto">
            <div className="overflow-y-auto max-h-[calc(100vh-320px)] sm:max-h-[calc(100vh-300px)] md:max-h-[calc(100vh-280px)]">
              {loading ? (
                <div className="text-white p-4 text-center">Loading time adjustment requests...</div>
              ) : error ? (
                ...
                <table className="w-full">
                  <thead className="sticky top-0 bg-[#2b2b2b] z-10">
                    <tr>
                      ...
                      <th className="text-white py-2 md:py-3 px-2 md:px-4 text-sm md:text-base text-center">Actions</th>
                    </tr>
                  </thead>
                  <tbody>
                    {currentRequests.length === 0 ? (
                      <tr>
                        <td colSpan="6" className="text-green-500 py-4 md:py-6 px-2 md:px-4 text-sm md:text-base text-center">
                          No time adjustment requests found
                        </td>
                      </tr>
                    ) : (
...
export default TimeAdjustmentPage;